...
const DistrictVillageDashboard = () => {
    ...
    // New states for all-district processing
    const [isProcessingAllDistricts, setIsProcessingAllDistricts] = useState(false);
    const [allDistrictsStats, setAllDistrictsStats] = useState({
        totalDistricts: 0,
        completedDistricts: 0,
        currentDistrictIndex: -1
    });
    ...
    const fetchVillages = async (districtCode, sroCode) => {
    ...
    const handleTehsilSelect = (tehsil) => {
        ...
        fetchVillages(selectedDistrict.code, tehsil.sroCode);
    };
    ...
    const fetchPropertyData = async (village = selectedVillage, tehsil = selectedTehsil, district = selectedDistrict, isBatchProcessing = false) => {
        ...
        try {
            ...
            addLogEntry({
                ...
                village: village,
                status: 'pending',
                message: `API Request: districtCode=${district.code}, sroCode=${tehsil.code}, gaonCode1=${village.value}`,
                requestBody
            });

            // Make the actual API call to fetch property data
            console.log(`Making API request for District: ${district.code}, SRO: ${tehsil.code}, GaonCode: ${village.value}`);
    ...
    // New function to process all districts automatically
    const startAllDistrictsProcessing = async () => {
        if (isProcessingAllDistricts || isProcessingBatch) return;

        // Set processing state
        setIsProcessingAllDistricts(true);

        // Log the start of all-districts processing
        addLogEntry({
            type: 'system',
            action: 'start_all_districts',
            message: `Starting automatic processing of all Districts with request queuing`
        });

        // Set initial statistics
        setAllDistrictsStats({
        ...
        console.log(`Starting processing of all districts: ${districts.length} districts found`);

        // Process each district one by one
        for (let i = 0; i < districts.length; i++) {
            if (!isProcessingAllDistricts) {
                console.log("Processing stopped by user");
                break; // Allow cancellation
            }

            const district = districts[i];
            console.log(`Processing district ${i + 1}/${districts.length}: ${district.code} - ${district.nameEn}`);

            // Update current district index
            setAllDistrictsStats(prev => ({
                ...prev,
            ...
            addLogEntry({
                type: 'system',
                action: 'process_district',
                district: district,
                message: `Processing District ${i + 1}/${districts.length}: ${getDisplayName(district)} (${district.code})`
            });
            ...
            // Fetch tehsils for this district
            await fetchTehsils(district.code);

            // Process each tehsil in the district
            for (const tehsil of tehsils) {
                if (!isProcessingAllDistricts) break;

                // Select current tehsil
                setSelectedTehsil(tehsil);

                // Fetch villages for this tehsil
                await fetchVillages(district.code, tehsil.sroCode);
                ...
                if (filteredVillages.length > 0) {
                    ...
                    await processVillagesWithQueue();
                    ...
                    addLogEntry({
                        ...
                        district: district,
                        tehsil: tehsil,
                        message: `Completed processing all villages for Tehsil: ${getDisplayName(tehsil)} (${tehsil.sroCode})`
                    });
                } else {
                    addLogEntry({
                        type: 'system',
                        ...
                        district: district,
                        tehsil: tehsil,
                        message: `No villages found for Tehsil: ${getDisplayName(tehsil)} (${tehsil.sroCode})`
                    });
                }
            }
            ...
            setAllDistrictsStats(prev => ({
                ...prev,
                completedDistricts: prev.completedDistricts + 1
            }));
        }

        addLogEntry({
            type: 'system',
            action: 'complete_all_districts',
            message: `Completed processing all Districts (${districts.length})`
        });

        // Reset processing state
        setIsProcessingAllDistricts(false);
    };

    // Modify the existing return statement to add the Auto Process All Districts button
    return (
        <div className="flex flex-row min-h-screen bg-gray-50">
            {/* Main Dashboard */}
            <div className="flex-1 p-6 overflow-y-auto">
                {/* Existing header and language toggle */}
                ...
                {/* Auto-Process All Districts Button */}
                <div className="mb-8 flex justify-between">
                    <button
                        onClick={startAllDistrictsProcessing}
                        disabled={isProcessingAllDistricts || isProcessingBatch}
                        className={`px-4 py-2 bg-purple-600 text-white rounded-lg shadow-md flex items-center space-x-2 hover:bg-purple-700 transition-colors ${isProcessingAllDistricts || isProcessingBatch ? 'opacity-60 cursor-not-allowed' : ''}`}
                    >
                                ...
                                <div className="flex items-center justify-between mb-6">
                                    ...
                                    <div className="flex gap-3">
                                        ...
                                        <button
                                            ...
                                            disabled={isProcessingBatch || filteredVillages.length === 0}
                ...
                {/* Add a new section for all districts processing info */}
                {isProcessingAllDistricts && (
                    <div className="fixed bottom-4 left-4 bg-white p-4 rounded-lg shadow-lg border-l-4 border-purple-500 max-w-md z-50">
                        <div className="font-medium text-gray-800 flex justify-between">
                            <span>{text.processingAllDistricts}</span>
                            <button
                                onClick={() => setIsProcessingAllDistricts(false)}
                                className="bg-red-600 text-white text-xs px-2 py-1 rounded"
                            >
                                {text.stopProcessing}
                            </button>
                        ...
                        <div className="font-medium text-gray-800 flex justify-between mt-2">
                            <span>{text.districtProgress}</span>
                            <span className="font-bold">
                                {allDistrictsStats.completedDistricts}/{allDistrictsStats.totalDistricts}
                            </span>
                        </div>
                        <div className="w-full bg-gray-200 rounded-full h-2.5 my-2">
                            <div
                                className="bg-purple-600 h-2.5 rounded-full transition-all"
                                style={{ width: `${(allDistrictsStats.completedDistricts / allDistrictsStats.totalDistricts) * 100}%` }}
                            ></div>
                        </div>
                        <div className="text-sm">
                            <span className="text-purple-600">
                                {text.remainingDistricts} {allDistrictsStats.totalDistricts - allDistrictsStats.completedDistricts}
                            </span>
                        </div>
                    </div>
                )}
...
export default DistrictVillageDashboard;